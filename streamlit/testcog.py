...
def load_data(path_to_geoparquet):
    # Load GeoParquet data into a GeoDataFrame
...
def read_geotiff(file_path, comment):  # Added 'comment' parameter
    # Read GeoTIFF file and display loading time
    start_time = time.time()
    dataset = rasterio.open(file_path)
    end_time = time.time()
    st.sidebar.text(f"{comment} GeoTIFF Loaded in: {end_time - start_time:.2f} seconds")

    # Additional performance indicator for GeoTIFF
    st.sidebar.text(f"{comment} GeoTIFF Resolution: {dataset.res[0]:.6f} x {dataset.res[1]:.6f} units/pixel")
...
# Function to display performance indicators for GeoTIFF
def display_geotiff_performance_indicators(geo_tiff_dataset):
    start_time = time.time()

    # Add performance indicators specific to GeoTIFF
    st.sidebar.text(f"GeoTIFF Resolution: {geo_tiff_dataset.res[0]:.6f} x {geo_tiff_dataset.res[1]:.6f} units/pixel")

    end_time = time.time()
    st.sidebar.text(f"Performance Indicators Calculated in: {end_time - start_time:.2f} seconds (GeoTIFF)")
...
def create_folium_map_with_mask_and_mouse_position_cog(raster_layer, geospatial_info, selected_attribute, zoom_level=5):
    start_time = time.time()
...
if __name__ == "__main__":
    st.title("Visualization of GeoTIFF and COG with Streamlit and Folium")
    ...
    cog_folium_map = create_folium_map_with_mask_and_mouse_position_cog(
    ...
    with col1:
        ...
        if geo_tiff_dataset:
            ...
            display_geotiff_performance_indicators(geo_tiff_dataset)
        else:
